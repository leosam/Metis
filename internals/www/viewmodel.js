...
function Action(data) {
  ...
  //DEBUG CODE, to check contents of expectedArgs in all cases
  /*
  var cache=[];
  var tmp = JSON.stringify(this.expectedArgs, function(key, value) {
  ...
  console.log("[created ACTION] new expectedArgs:"+tmp);
  */

  //#### here's the specific part for chart UI (on each action) ###
  this.addCircle = function(arg, circle) {
    if (this.expectedArgs[arg] != null) {
      if (this.expectedArgs[arg].circle != undefined && this.expectedArgs[arg].circle != null )
        console.log("ERROR [addCircle] there's already a circle there" );
      this.expectedArgs[arg].circle = circle;
    }
    else {
      console.log("ERROR "+arg+" doesn't exist and leads to null ("+this.name+")");
    }
  }
}
